...
${ORDER_AND_PAGE.replace('$LIMIT', '$1').replace('$OFFSET', '$2')}`;
  return { text, values: [clampLimit(limit), clampOffset(offset)] };
}
...
${ORDER_AND_PAGE.replace('$LIMIT', '$2').replace('$OFFSET', '$3')}`;
  return { text, values: [externalId, clampLimit(limit), clampOffset(offset)] };
}

export async function listAllImages(pool: Pool, limit: number, offset: number): Promise<ImageListRow[]> {
  const { text, values } = buildAdminListQuery(limit, offset);
  const result = await pool.query(text, values);
  return result.rows as ImageListRow[];
}
...
export async function listImagesForExternalId(
  ...
  const { text, values } = buildUserListQuery(externalId, limit, offset);
  const result = await pool.query(text, values);
  return result.rows as ImageListRow[];
}
