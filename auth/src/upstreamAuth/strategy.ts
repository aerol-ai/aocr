...
export async function validateWrappedUpstream(
  ...
  const repoPath = extractRepoFromScope(scope) ?? '';

  // Fast path: cached proof covers this exact identity + repo.
  if (deps.proofCache.get(identity, repoPath)) {
    return { identity, cacheHit: true };
  }

  // Slow path: probe the upstream.
  const probe = (deps.resolveProbe ?? getProbeForHost)(creds.upstreamHost);
  const result = await probe.probe(creds, repoPath);
  if (result.ok === false) {
  ...
  deps.proofCache.record(identity, repoPath, result.upstreamBearer);
